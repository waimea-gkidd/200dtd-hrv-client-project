...
# copied from 'list things' page in flask-intro
#-----------------------------------------------------------
@app.get("/clients")
def clients_list():
    with connect_db() as db:
...
def meeting_form():
    ...
    with connect_db() as db:
        ...
        # NEW: do this *before* the context closes
        result = db.execute('SELECT id, priority FROM "follow-ups" ORDER BY id DESC', [])
        priorities = result.rows

    return render_template("pages/meeting-form.jinja", clients=clients, priorities=priorities)
...
def followups_list():
    with connect_db() as db:
        ...
        #same as meeting-form
        result = db.execute("SELECT id, name FROM clients ORDER BY name ASC", [])
        clients = result.rows

    return render_template("pages/follow-ups.jinja", meetings=meetings, clients=clients)
